...
Thank you for the kind comments on the Google form and evaluations from last semester. I'm really looking forward to having you all as students. As a teacher, I don't always see the impact I have, but your feedback made my year. After moving from the world of corporate performance evaluations to reading your feedback, it helped me decide to stick with teaching for at least the next few years before I start considering something new.

For the first time in my career, I can genuinely say I’m excited to work with you all this upcoming semester. Please never hesitate to pitch ideas for improving the class :^)
...
As of August 25, 2024 the latest version of JavaScript is ECMAScript 2023, officially known as ES14. Some of the highlights include:
...
I’m linking my GitHub repo here on purpose. Feel free to explore it—it's a great learning opportunity if you're curious about how I code. And if you think my code could be better, go ahead and file a [GitHub Issue](https://github.com/elizabeth-poggie/elizabeth-poggie/issues) on my repo. on my repo. If you’re familiar with Pull Requests, you’re welcome to create a [PR](https://github.com/elizabeth-poggie/elizabeth-poggie/pulls). The code is open-source, and I’m always open to contributions ;)
...
What makes this different to the arrays you guys are comfortable with? They can be of any kind of data, even mixed for those of you who want to live dangerously
...
Let's explore the beauty of HTML, CSS, and JavaScript work together. For the following Pen and this time, instead of adding a name to a list, you will be adding a url to a gallery.
...
// 👉 Step 0 ) Initialize the Array with some items
// ------------------------------------------------
// The 'clowder' array contains a list of initial cat image URLs.
var clowder = [
  "https://miro.medium.com/v2/resize:fit:1000/1*6ZpFPuftEFRVNtDL5IMXXw.jpeg",
  "https://i.pinimg.com/1200x/c3/0a/ae/c30aae89cfdb0dff8af33fe4fdea348a.jpg",
  "https://i.pinimg.com/236x/29/44/b9/2944b90350f444679dff98674fa36ec1.jpg",
];
...
// Iterate over each item in the 'clowder' array.
clowder.forEach(function (item) {
  // Create an image element and set its source to the item URL.
  var img = $("<img>").attr("src", item).attr("alt", "Cat");
...
// ---------------------------------------
function addCat() {
  var newItem = prompt("Add a new cat URL to the clowder");
  // Check if the user provided a URL (i.e., the input is not empty).
  if (newItem) {
    // Add the new image URL to the 'clowder' array.
    clowder.push(newItem);
    // Append the new image to the gallery.
    var img = $("<img>").attr("src", newItem).attr("alt", "Cat Image");
    gallery.append(img);
  }
}
```
...
What if the user inputs something that isn’t a URL? Instead of displaying a proper image, it will show the alt attribute. This is where input validation comes in handy. We’ll discuss this topic more in Week 3 :^)
