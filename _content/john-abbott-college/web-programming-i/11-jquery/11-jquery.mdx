...
If you were determined enough you could copy the source code from [github](https://github.com/jquery/jquery) into your header `<script>` tag and that would also work however not the best practice.

Why am i mentioning this? In JavaScript, a library is a collection of pre-written code that provides helpful functions and tools to solve common problems without having to write everything from scratch. It would be like taking all your assignment functions and saving it in one place to be `import`ed in every project.

In the case with jQuery, instead of accessing the library with a CDN which makes your code cleaner, you could "plagiarize" the source code functions by just pasting them directly in the header where you define it from "scratch" yourself.

But that takes too long, stick to the CDN method.
