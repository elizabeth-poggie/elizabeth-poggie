...
`DOMContentLoaded` prepares the DOM and ensures it's fully loaded before event listeners (and therefore bubbling) can be used.

## Why is this important for Event Bubbling?

`DOMContentLoaded` ensures that the DOM structure is ready and accessible, meaning that event listeners (such as those needed for event bubbling) can be attached safely.

![demo](./assets/6.png)

If there is no DOM, there is no `target` which means there are no elements to attach an event listener to.

## How do we revise our example to be safe?
