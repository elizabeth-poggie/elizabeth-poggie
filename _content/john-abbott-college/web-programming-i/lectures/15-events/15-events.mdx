...
Before reaching the source of the event, or _target_ element, an event is captured while descending through the DOM tree. This process is known as "event capturing", where the event starts from the root and travels down to the _target_ element.

## How do events propagate?

Once the event has been captured and we reach the _target_ element, it activates that element's event handler.
