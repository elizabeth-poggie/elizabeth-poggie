...
The DOM is structured like a tree, consisting of parent and child elements. When an event occurs on a specific _target_ element, such as a button, it first activates that element's event handler.

![tree](./assets/image.png)

After executing the handler on the target element, the event "bubbles" up to its parent element, then to the grandparent, and so on, until it reaches the root of the DOM. This allows parent elements to also respond to events that happen on their children and becomes particularly evident when we have nested event handlers.

## How do we identify the target?

Before reaching the target element, an event can also be captured while descending through the DOM tree. This process is known as event capturing, where the event starts from the root and travels down to the _target_ element.
...
Once the event is triggered, it enters the capturing phase, where it travels from the root of the DOM down to the target element:
...
- Keeping it Simple 👉 Having a single listener for a group of elements simplifies the codebase.
...
Write the worst JavaScript code you can that still compiles. Refer to the [official documentation](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Strict_mode) for inspiration.
