...
We return to concepts from our beloved User Interfaces where we begin to build in complexity by taking the JavaScript logic we have learnt so far and combine it with `HTML` and `CSS`

The document object is provided by the browser and is a representation of the HTML document. This document object is not part of JavaScript, but is expected to already exist and be freely accessible to JavaScript Code.
