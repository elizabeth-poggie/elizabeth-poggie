...
title: "Operating Systems and Software Applications"
created: "2024-09-04"
coverSrc: "./assets/"
---
...
In the early days, if an error occurred in the OS, it would trigger a routine called `PANIC`, causing the machine go into a vegetable like state.
...
In lea you will find a `Exercise 1 - OneDrive.pdf` file. Download the file and complete all the steps.

# Microsoft Word

Microsoft Word is a powerful word-processing program that is widely popular in industry. It allows you to create a variety of **documents**, including letters, resumes, and reports.
...
When you get started with editing a document, you will notice a Ribbon at the top of the screen

![ribbon](./assets/2.PNG)

The ribbon is divided into different Tabs:
...
Each Tab has different sections.

![section](./assets/3.png)

For example the "Home" tab has some of the following options:
...
If you are not able to find a command you need, the search bar can help. It works just like a regular search bar. Type what you're looking for, and a list of options will appear.
...
- Zoom in an out of the doc to your preferences.
- Change the document view mode to one of three options:
...
- You can start a blank document or a document from a template.

From an existing doc:

- Click on the **File** tab.
- View a doc you know and love, choose open

## Saving Documents

- Can use the save command on the **Quick Access Toolbar.**

- If it is a new document go to File tab and choose where you want to save your file.

- You can save a different version maybe in a different location go to the File tab and select **Save a Copy**.

## Exporting Documents

Word documents are saved in the `.docx`file type/ However, there may be times when you need to use another file type, such as a `.pdf`.
...
![section](./assets/Tables-lab_schedule.png)

Download the image `Tables-lab_schedule.png` in LEA. Try to re-create the table in the image using a table in Word, reproducing it as closely as possible.
