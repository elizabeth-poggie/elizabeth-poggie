...
Ok so now we know how to interact with an output device (monitor) that displays storage information in a way that makes sense to humans. But what is happening under the hood? While we operate on files and folders that makes sense to us, what does the CPU, or the computer's brain operate on that makes sense to the computer?

## RAM

Like the memory in the human brain, computer memory stores data. Also like human memory, there is both short-term (temporary) and long-term (permanent) memory storage. The `RAM` (Random Access Memory), is used to temporarily hold information while the CPU is processing it. Think of this as the computer’s work area, it works similarly to your "short term" memory.
...
The more RAM you have, the more programs you can open at the same time which is why gamers are fixated on getting a lot of RAM when getting a computer. They need a lot of "short term memory" to be able to run these massive simulations smoothy

However, once the computer is powered off, all information is erased from the RAM. So if we need RAM to run our games, programs, and surf the web, how do we still get access to this information after we have turned off and back on our computer?
